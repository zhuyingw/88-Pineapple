...
# ── HYDROGEL_PACK — Stationary wall-mid market maker ─────────────────────────
class HydrogelStrategy(MarketMakingStrategy):
    """
    HYDROGEL_PACK: Round 3 stationary spot product.

    Notebook profile:
      wall_mid mean:   ~9990.8
      wall_mid std:    ~31.9
      spread:          ~15.7 ticks
      ADF p-value:     ~0.0
    ...
    def get_fair_value(self, ctx: ProductContext, state: TradingState) -> Optional[float]:
        """
        Fair value = live wall_mid plus a small pull toward an adaptive EMA.

        The live wall_mid remains the main signal.  The EMA only nudges fair value
        when price is temporarily away from its recent centre, and the nudge is
        capped so a real directional change is not faded aggressively.
        """
        wm = ctx.wall_mid
        if wm is None:
            return None
        ...
        self._update_ema(wm)
        if self._ema_mid is None or self._ema_samples < self._MIN_MEAN_SAMPLES:
            return wm

        raw_tilt = (self._ema_mid - wm) * self._MEAN_REVERSION_STRENGTH
        tilt = max(-self._MEAN_REVERSION_CAP, min(self._MEAN_REVERSION_CAP, raw_tilt))
        return wm + tilt

    def _update_ema(self, wall_mid: float) -> None:
        """Update the adaptive centre used by the mild mean-reversion tilt."""
    ...
    def _inventory_skewed_fair_value(self, ctx: ProductContext, fair_value: float) -> float:
        ...
        The first 50% of position capacity is a dead zone: HYDROGEL is stationary
        with a wide spread, so small inventory is acceptable if it earns edge.
        Beyond that, skew grows quadratically and pushes quotes toward flattening:
        long inventory lowers fair value; short inventory raises it.
        """
        if ctx.limit <= 0:
            return fair_value
    ...
    def act(self, ctx: ProductContext, state: TradingState) -> None:
        ...
        Primitives.post_passive_quotes(self, ctx, skewed_fv, self._MAKE_EDGE)
