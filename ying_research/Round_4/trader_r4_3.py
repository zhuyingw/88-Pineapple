...
OPTION_SYMBOLS    = [f"VEV_{k}" for k in OPTION_STRIKES]
...
# ── Round-4 named-trader signals ───────────────────────────────────────────────
# Round 4 trade tapes carry buyer/seller names directly.  Per
# `round4_informed_trader_analysis.ipynb` §3:
#   - Mark 55: VELVETFRUIT_EXTRACT, fwd_100 mean=+0.71 / t=+2.72 / hit=51.9%
#   - Mark 67: VELVETFRUIT_EXTRACT, fwd_100 mean=+1.54 / t=+2.10 / hit=54.5%
# Following both at H≈200 grid ticks (Strategy A) → Sharpe +2.32, PnL +1167.5.
# Note: Mark 67's edge *reverses* at H=10 000 (t=−1.89), so do NOT hold his
# bias beyond the lifetime cap.
INFORMED_MARKS_VELVET: Tuple[str, ...] = ("Mark 55", "Mark 67")
...
class Primitives:
    ...
    @staticmethod
    def check_named_signal_persistent(
        symbol: str,
        state: TradingState,
        traders: Tuple[str, ...],
        ...
        """
        Multi-trader, lifetime-aware sibling of `check_informed_signal`.

        Why a separate method:
        - `check_informed_signal` only sees this tick's market_trades, which is
          fine for the always-active Olivia signal.
        - Round-4 Marks (e.g. Mark 55, Mark 67) trade sparsely (~1 trade per
          25 ticks).  We need to *remember* the latest BUY / SELL timestamp
          across ticks and decay the signal once it grows stale.

        Side effects (in-place):
        - `last_buy[trader]` / `last_sell[trader]` are updated to the most
          recent timestamp seen this tick.

        Returns the most recent BUY-vs-SELL across `traders`, considered
        active only if its timestamp is within `lifetime` ts units of now.
        """
        now = state.timestamp
        cutoff = now - lifetime
        ...
        for t in (state.market_trades.get(symbol, [])
                  + state.own_trades.get(symbol, [])):
            if t.buyer in traders:
                if t.timestamp > last_buy.get(t.buyer, -1):
                    last_buy[t.buyer] = t.timestamp
            if t.seller in traders:
                if t.timestamp > last_sell.get(t.seller, -1):
                    last_sell[t.seller] = t.timestamp

        latest_buy  = max((ts for ts in last_buy.values()  if ts >= cutoff),
                          default=None)
        latest_sell = max((ts for ts in last_sell.values() if ts >= cutoff),
                          default=None)
        ...
        if latest_sell is not None and latest_buy is None:
            return Signal.SHORT
        return Signal.LONG if latest_buy > latest_sell else Signal.SHORT
...
class TomatoesStrategy(MarketMakingStrategy):
    ...
    def load(self, data: Any) -> None:
        if isinstance(data, dict):
            ...
            self.signal = Signal(data)
# ------------------------------------------------------------
# PEPPER_ROOT strategy - Wall-mid market making as Osmium

class PepperRootStrategy(MarketMakingStrategy):
    """
    INTARIAN_PEPPER_ROOT: CONSERVATIVE, pure market making around wall-mid fair value, 
    if it deviates a lot from fv.

    Keep this aligned with the standard MarketMakingStrategy flow used by
    OsmiumStrategy:
      1. take favourable orders vs fair value
      2. zero-EV flush at fair value
      3. post passive quotes around fair value

    No drift prediction, directional accumulation, or stateful biasing.
    """
    ...
    def get_fair_value(self, ctx: ProductContext, state: TradingState) -> Optional[float]:
        # Docstring says "no drift prediction" — wall_mid is the fair value.
        # If wall_mid is unavailable for some tick, fall back to simple mid.
        return ctx.wall_mid if ctx.wall_mid is not None else ctx.mid_price()
...
#         #            self.buy(price, qty)
#         ###################################
#         #--------> changing to NOT ALWAYS take the bot's ask (only when it's within edge, 
#         # if above fv), check to see if it's better pnl.
...
#                 if remaining <= 0:
#                     break
#                 if price > fv_int + self._MAKE_EDGE:
#                     break
#                 qty = min(volume, remaining)
#                 if qty > 0:
#                     self.buy(price, qty)
...
# ── HYDROGEL_PACK — Mean-reversion-skewed market maker ───────────────────────
class HydrogelStrategy(MarketMakingStrategy):
    """
    HYDROGEL_PACK: stationary spot product with wide spread.

    R3_3 experiment:
      - Keep the r3_2 market-making structure because HYDROGEL's main edge is
        still spread capture around a stable wall_mid.
      - Use an adaptive EMA as a mean-reversion anchor.
      - When wall_mid is below EMA, shade fair value up and make bids tighter.
      - When wall_mid is above EMA, shade fair value down and make asks tighter.

    The mean-reversion signal biases quoting; it does not replace the market
    maker. This avoids the 20/40 cross version's problem of carrying a large
    one-sided position for too long.
    """
    ...
    def get_fair_value(self, ctx: ProductContext, state: TradingState) -> Optional[float]:
        """
        Fair value starts at live wall_mid and is shifted toward the EMA anchor.
        Positive dev means price is below EMA, so we shade fair value upward.
        """
        wm = ctx.wall_mid
        if wm is None:
            return None
    ...
    def _update_ema(self, wall_mid: float) -> None:
        """Update the slow mean-reversion anchor."""
        ...
        self._ema_mid = (
            self._MEAN_ALPHA * wall_mid
            + (1.0 - self._MEAN_ALPHA) * self._ema_mid
        )
        self._ema_samples += 1
    ...
    def _inventory_skewed_fair_value(self, ctx: ProductContext, fair_value: float) -> float:
        """
        Inventory risk control layered after the mean-reversion fair-value tilt.
        Large long inventory lowers fair value; large short inventory raises it.
        """
        if ctx.limit <= 0:
            return fair_value
        ...
        skew = math.copysign(
            self._INVENTORY_SKEW_TICKS * (excess_ratio ** self._SKEW_POWER),
            inventory_ratio,
        )
    ...
    def _quote_edges(self) -> Tuple[float, float]:
        """
        Side-specific quote bias:
          - dev > 0: price is below EMA, tighten bid and widen ask.
          - dev < 0: price is above EMA, widen bid and tighten ask.
        """
    ...
    def _near_fair_flatten(self, ctx: ProductContext, fair_value: float) -> None:
        """Trim inventory near fair value without dominating the MR quote bias."""
...
class VelvetExtractStrategy(Strategy):
    """
    VELVETFRUIT_EXTRACT market maker.

    Fair-value stack:
      fair = mid
           + microprice tilt
           + EMA mean-reversion tilt
           - inventory skew

    Mark 55 / Mark 67 are tracked as a short-lived signal and used as a
    size/risk overlay rather than a fair-value shift.  This keeps the observed
    indicator without degrading the baseline quote price.

    KEY CHANGE from trader_r4_2.py: Use Mark as risk brake: 
      Mark LONG  -> larger bid size, smaller ask size
      Mark SHORT -> smaller bid size, larger ask size
    """
    HALF_WIDTH  = 1
    MICRO_COEFF = 0.55
    INV_SKEW    = 4.0
    ...
    # EMA mean reversion.  Best mark0 variant used MR_COEFF=0.20, MR_CAP=5.0.
    EMA_ALPHA       = 0.005
    MIN_EMA_SAMPLES = 60
    MR_COEFF        = 0.20
    MR_CAP          = 5.0
    ...
    MARK_SIZE_MULT:         float = 0.25

    def __init__(self) -> None:
        super().__init__(VELVET)
        self._mark_last_buy:  Dict[str, int] = {}
        self._mark_last_sell: Dict[str, int] = {}
        self._ema_mid: Optional[float] = None
        self._ema_samples: int = 0

    def _update_ema(self, mid: float) -> Optional[float]:
        """Update the slow mean-reversion anchor and return the previous EMA."""
        prev = self._ema_mid
        if self._ema_mid is None:
            self._ema_mid = mid
            self._ema_samples = 1
            return prev
        self._ema_mid = self.EMA_ALPHA * mid + (1.0 - self.EMA_ALPHA) * self._ema_mid
        self._ema_samples += 1
        return prev

    @staticmethod
    def _microprice(ctx: ProductContext) -> Optional[float]:
        if ctx.best_bid is None or ctx.best_ask is None:
            return None
        bid_size = ctx.buy_orders.get(ctx.best_bid,  0)
        ask_size = ctx.sell_orders.get(ctx.best_ask, 0)   # already positive in ctx
        total = bid_size + ask_size
        if total <= 0:
            return ctx.mid_price()
        return (ctx.best_bid * ask_size + ctx.best_ask * bid_size) / total

    def act(self, ctx: ProductContext, state: TradingState) -> None:
        # Update Mark memory before pricing so same-tick flow can affect quotes.
        mark_sig = Primitives.check_named_signal_persistent(
            self.symbol, state,
            traders=self.INFORMED_MARKS,
            last_buy=self._mark_last_buy,
            last_sell=self._mark_last_sell,
            lifetime=self.MARK_SIGNAL_LIFETIME,
        )
        ...
        mid   = ctx.mid_price()
        micro = self._microprice(ctx)
        if mid is None or micro is None:
            return

        position  = ctx.position
        inv_ratio = position / ctx.limit if ctx.limit > 0 else 0.0
        prev_ema = self._update_ema(mid)

        # Positive mr_dev means price is below the slow anchor (up-reversion).
        mr_dev = 0.0
        mr_tilt = 0.0
        if prev_ema is not None and self._ema_samples >= self.MIN_EMA_SAMPLES:
            mr_dev = prev_ema - mid
            raw_mr = self.MR_COEFF * mr_dev
            mr_tilt = max(-self.MR_CAP, min(self.MR_CAP, raw_mr))

        fair = (
            mid
            + self.MICRO_COEFF * (micro - mid)
            + mr_tilt
            - self.INV_SKEW * inv_ratio
        )

        # Keep fair-value mark bias disabled for Version B.  `mark_used` below
        # records whether the Mark signal affected quote sizes.
        ...
        elif mark_sig == Signal.SHORT and mr_dev < -self.MR_DEADZONE:
            fair -= self.MARK_BIAS_TICKS
        ...
        # Opportunistic take when the book crosses our fair by TAKE_EDGE.
        if ctx.best_ask <= fair - self.TAKE_EDGE:
            avail = self._book.sell_orders.get(ctx.best_ask, 0)   # positive
            qty   = min(avail, ctx.max_buy - self._buy_spent, self.MAX_TAKE)
            if qty > 0:
                self.buy(ctx.best_ask, qty)

        if ctx.best_bid >= fair + self.TAKE_EDGE:
            avail = self._book.buy_orders.get(ctx.best_bid, 0)
            qty   = min(avail, ctx.max_sell - self._sell_spent, self.MAX_TAKE)
            if qty > 0:
                self.sell(ctx.best_bid, qty)

        projected  = ctx.position + self._buy_spent - self._sell_spent
    ...
    def save(self) -> Dict[str, Any]:
        return {
            "mark_buy":  self._mark_last_buy,
            "mark_sell": self._mark_last_sell,
            "ema_mid": self._ema_mid,
            "ema_samples": self._ema_samples,
        }
    ...
    def load(self, data: Any) -> None:
        if not isinstance(data, dict):
            return
        self._mark_last_buy  = {
            str(k): int(v) for k, v in (data.get("mark_buy")  or {}).items()
        }
        self._mark_last_sell = {
            str(k): int(v) for k, v in (data.get("mark_sell") or {}).items()
        }
        ema_mid = data.get("ema_mid")
        self._ema_mid = float(ema_mid) if ema_mid is not None else None
        self._ema_samples = int(data.get("ema_samples", 0))
...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self._strategies: Dict[str, Strategy] = {
            ...
            HYDROGEL:    HydrogelStrategy(),
            VELVET:      VelvetExtractStrategy(),
