"""
IMC Prosperity 5 — Trader v2 (Phase 1 + Phase 2)
=================================================

Adds 4 cointegrated-pair strategies on top of the v1 PEBBLES basket arbitrage.
All strategy specs (β/α, half-life, thresholds, expected PnL) come from
ying_research/Round_5/round5_group_analysis.ipynb §6 + §7.
...
Strategies registered in Trader.__init__:
  ...
  Phase 2 — Cointegrated pair z-score reversion (new in v2):
    ...
    - Exit   : |z| ≤ 0.3 → flat
    - Freeze : |z| > 4.0 → flat + freeze; auto-unfreeze when |z| ≤ 0.3
    ...
    - Combined backtested PnL:  +103,885 over 30,000 ticks (sum of gated runs).

Total v2 expected backtested PnL:  ~ +149,000 over 30,000 ticks (notebook
simulator with mid-price fills).  A spread-crossing-only harness on the same
historical CSVs yields ~ +16,000 (worst case: passive quotes never fill).
The official Prosperity backtester models bot reactions and should land
between the two.  Submit and read the truth from the backtester.
...
class PebblesBasketStrategy(MultiSymbolStrategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        for sym, c in ctxs.items():
            ...
            max_bid = int(synth_fair) - int(self._MAKE_EDGE)
            min_ask = int(synth_fair) + int(self._MAKE_EDGE)
...
class PairZScoreStrategy(MultiSymbolStrategy):
    """
    Cointegrated-pair z-score reversion strategy.
    ...
    Per-tick logic:
        ...
        4. State machine on z_t:
             |z|  > Z_FREEZE  -> POSITION_FROZEN  (close both legs)
             z   <= -Z_IN     -> POSITION_LONG_RES  (long  +10 of a, short -round(beta·10) of b)
             z   >= +Z_IN     -> POSITION_SHORT_RES (short -10 of a, long  +round(beta·10) of b)
             |z|  <= Z_OUT    -> POSITION_FLAT     (close both legs)
             frozen + |z| <= Z_OUT -> POSITION_FLAT (auto-unfreeze)
    ...
    Persisted state per pair (~50 bytes JSON, 4 pairs total ≈ 250 bytes):
        ewma_mean, ewma_var, n, position_state
    """
    ...
    def _next_state(self, z: float) -> int:
        ...
        if self._position_state == self.POSITION_FROZEN:
            return self.POSITION_FLAT if abs(z) <= self.Z_OUT else self.POSITION_FROZEN
    ...
    def act(self, state: TradingState) -> None:
        ...
        logger.print(
            ...
            "state", target, "tgt_a", target_a, "tgt_b", target_b,
...
class Trader:
    """Round 5 entry point. v2 ships PEBBLES basket + 4 cointegrated pairs."""
